...
double curvature_steer_control::steering_control(pt_control_state_t state, path_point_t target_point)
{
    ...
    if (fabsf(state.v) > 0.01) {
        ...
        while (count <= 10) {
            ...
            for (int i = 0; i < 10; i++) {
                double v_to_g_slope_diff_angle = std::atan(this->lr * std::tan(current_state.steer) / this->wheel_base);
            ...
            double temp_yaw_error = path_tracker::pi_to_pi(target_yaw - current_state.yaw);
