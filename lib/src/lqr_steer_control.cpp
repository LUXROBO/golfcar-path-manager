...
void lqr_steer_control::generate_spline(ControlState init_state, std::vector<WayPoint> waypoints, float target_speed, float ds)
{
...
int lqr_steer_control::calculate_nearest_index(ControlState state, std::vector<Point> points, int pind, float& min_distance_ref)
{
    ...
    float dxl = points[min_index].x - state.x;
    float dyl = points[min_index].y - state.y;
...
int lqr_steer_control::lqr_steering_control(ControlState state, float& steer, float& pe, float& pth_e)
{
    float e = 0;
    this->target_ind = this->calculate_nearest_index(state, this->points, this->target_ind, e);

    float k = this->points[this->target_ind].k;
    float v = state.v;
...
bool lqr_steer_control::update(float dt) {
    ...
    float calculated_accel = this->path_pid.calculate(this->state.v);
    // state update
...
float lqr_steer_control::calculate_error()
{
    float e = 0;
    float distance1 = 100;
    float distance2 = 100;
...
ControlState lqr_steer_control::update_state(ControlState state, float accel, float steer_delta, float dt)
{
